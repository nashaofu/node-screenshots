...
async function main() {
  ...
  let monitor = runWithTime(() => Monitor.fromPoint(100, 100), 'Monitor.fromPoint(100, 100)')

  let image = runWithTime(() => monitor.captureImageSync(), 'monitor.captureImageSync();')
  saveImage(`monitor-${monitor.id()}.jpeg`, image.toJpegSync())

  let captureImagePromise = runWithTime(() => monitor.captureImage(), 'monitor.captureImage()')
