...
test("screenshots.capture()", async (t) => {
    let screenshots = Screenshots.fromPoint(100, 100);
    let buffer = await screenshots.capture(false);
    t.true(buffer !== null);
});

test("screenshots.captureSync()", (t) => {
    let screenshots = Screenshots.fromPoint(100, 100);
    let buffer = screenshots.captureSync(true);
    t.true(buffer !== null);
});

test("screenshots.captureArea(0, 0, 10, 10)", async (t) => {
    let screenshots = Screenshots.fromPoint(100, 100);
    let buffer = await screenshots.captureArea(0, 0, 10, 10, true);
    t.true(buffer !== null);
});

test("screenshots.captureAreaSync(0, 0, 10, 10)", (t) => {
    let screenshots = Screenshots.fromPoint(100, 100);
    let buffer = screenshots.captureAreaSync(0, 0, 10, 10, false);
    t.true(buffer !== null);
});
